...
pub fn build(b: *std.Build) void {
    ...
    lib_mod.addIncludePath(b.path("common"));
    lib.installHeader(b.path("ui.h"), "ui.h");
    lib_mod.addCMacro("libui_EXPORTS", "");
    ...
    if (isDarwin) {
        ...
        lib_mod.addIncludePath(b.path("darwin"));
        lib_mod.linkFramework("Foundation", .{});
        lib_mod.linkFramework("Appkit", .{});
        lib_mod.addSystemIncludePath(b.path("Cocoa"));
        lib_mod.addCSourceFiles(.{
        ...
        lib_mod.linkSystemLibrary("kernel32", .{});
        lib_mod.linkSystemLibrary("gdi32", .{});
        lib_mod.linkSystemLibrary("comctl32", .{});
        lib_mod.linkSystemLibrary("uxtheme", .{});
        lib_mod.linkSystemLibrary("msimg32", .{});
        lib_mod.linkSystemLibrary("comdlg32", .{});
        lib_mod.linkSystemLibrary("d2d1", .{});
        lib_mod.linkSystemLibrary("dwrite", .{});
        lib_mod.linkSystemLibrary("ole32", .{});
        lib_mod.linkSystemLibrary("oleaut32", .{});
        lib_mod.linkSystemLibrary("oleacc", .{});
        lib_mod.linkSystemLibrary("uuid", .{});
        lib_mod.linkSystemLibrary("windowscodecs", .{});
        ...
        if (is_dynamic) {
            // Embed rpath so executables can find the lib
            lib.rdynamic = true;
        }
        lib_mod.addIncludePath(b.path("unix"));
        lib_mod.addCSourceFiles(.{
    ...
    inline for (example_names) |name| {
        ...
        // if (is_dynamic and target.result.os.tag == .windows) {
        //     exe.addRPath("$ORIGIN");
        // }
